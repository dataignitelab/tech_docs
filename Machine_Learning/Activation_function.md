...
- 3개 이상의 다중 분류문제에서 모델의 마지막 레이어의 출력 값에 적용하는 활성화 함수로 많이 활용된다.
  ...
  - 일반적으로 Softmax를 적용하지 않은 모델의 아웃풋은 데이터의 크기가 정규화되어 있지 않아서 해당 값의 크기가 어느정도의 의미를 가지고 있는지 판단하기 어렵다. 따라서 Softmax를 적용하여 합이 1인 상태로 만들어서 상대적인 비교가 가능하도록 하는 것이 기본 개념이다.
